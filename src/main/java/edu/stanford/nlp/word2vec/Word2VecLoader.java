...
import edu.stanford.nlp.cache.TransferMap;
...
public class Word2VecLoader {
    ...
    public static TransferMap<String,double[]> loadDataAsynch(String path) throws IOException {
        ...
        TransferMap<String,double[]> embeddings = new TransferMap<>();

        // Spin off a thread to do the reading asynchronously
        Thread reader = new Thread(new EmbeddingReaderThread(input, embeddings));
        reader.start();

        return embeddings;
    }

    private static class EmbeddingReaderThread implements Runnable {
        Input input;
        TransferMap<String,double[]> embeddings;

        public EmbeddingReaderThread(Input input, TransferMap<String,double[]> embeddings) {
            this.input = input;
